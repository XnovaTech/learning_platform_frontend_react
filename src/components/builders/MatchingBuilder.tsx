...
export default function MatchingBuilder({ initial, onChange }: Props) {
  ...
  const handleLeftChange = (idx: number, val: string) => setLeft((s) => s.map((it, i) => (i === idx ? val : it)));

  const handleRightChange = (idx: number, val: string) => setRight((s) => s.map((it, i) => (i === idx ? val : it)));
    ...
    <div className=" grid grid-cols-2 gap-4">
      ...
      <div>
        <div className=" flex items-center justify-between mb-2">
          ...
          <Button size="sm" onClick={handleAddLeft}>
            Add
          </Button>
        </div>
            ...
            <div className="space-y-2">
              {left.map((val, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <SortableItem id={`L-${idx}`} value={val} onChange={(v: string) => handleLeftChange(idx, v)} onRemove={() => handleRemoveLeft(idx)} />
                </div>
              ))}
            </div>
          </SortableContext>
      ...
      <div>
        <div className=" flex items-center justify-between mb-2">
          <div className="font-medium">Right Items</div>
          <Button size="sm" onClick={handleAddRight}>
            Add
          </Button>
        </div>
        ...
        <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEndRight}>
          <SortableContext items={right.map((_, i) => `R-${i}`)} strategy={verticalListSortingStrategy}>
            <div className="space-y-2">
              {right.map((val, idx) => (
                <SortableItem key={idx} id={`R-${idx}`} value={val} onChange={(v: string) => handleRightChange(idx, v)} onRemove={() => handleRemoveRight(idx)} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
