...
import type { TaskType, DragDropExtraData, MatchingExtraData, UpdateCourseExamPayloadType, ParagraphDropdownData } from '@/types/task';
...
import { updateCourseExam, uploadImage } from '@/services/courseExamService';
import { toast } from 'sonner';
import { LessonTaskQuill } from '@/components/ui/lesson-task-quill';


type Props = {
    initial?: UpdateCourseExamPayloadType | null;
    examId: number;
    onClose?: () => void;
    refetch: () => void;
}
...
export default function UpdateExam({initial, examId, onClose, refetch}: Props){
    ...
    const [selectedExamType, setSelectedExamType] = useState(initial?.exam_type ?? '');
    const [selectedExamSection, setSelectedExamSection] = useState(initial?.exam_section ?? '');
    ...
    const createMutation = useMutation({
        mutationFn: (payload: UpdateCourseExamPayloadType) => updateCourseExam(examId, payload),
        onSuccess: async () => {
        ...
        onError: (error: any) => {
            toast.error(error?.message || `Failed to update exam question`);
        },
    });
    ...
    const onSubmit = async (e: React.FormEvent) => {
      ...
      const payload: UpdateCourseExamPayloadType = {
         ...
         <div className="space-y-5 p-5 border  bg-white shadow w-full">
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className='col-span-2'>
                  <Label className="font-medium mb-2">Exam Type</Label>
                  <Select value={selectedExamType} onValueChange={(v) => setSelectedExamType(v)}>
                    <SelectTrigger className="w-full">
              ...
              <Button type="submit" className="w-full" onClick={onSubmit} disabled={createMutation.isPending}>
                  {createMutation.isPending ? 'Updating...' : 'Update Exam Question'}
              </Button>
            </div>
    )
}