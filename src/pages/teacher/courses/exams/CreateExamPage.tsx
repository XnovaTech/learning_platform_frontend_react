...
import type { TaskType, DragDropExtraData, MatchingExtraData, CreateCourseExamPayloadType, UpdateCourseExamPayloadType, ParagraphDropdownData } from '@/types/task';
...
export default function CreateExamPage({initial, examId}: Props){
    ...
    const createMutation = useMutation({
        mutationFn: examId ? (payload: UpdateCourseExamPayloadType) => updateCourseExam(examId, payload) : createCourseExam,
        onSuccess: async () => {
            toast.success(`Exam question ${examId ? 'updated' : 'created'} successfully`);
            await queryClient.invalidateQueries({ queryKey: ['courseExams'] });
        },
        onError: (error: any) => {
            toast.error(error?.message || `Failed to ${examId ? 'update' : 'create'} exam question`);
        },
    });
